...
docker run -d --name brevoemail-sender -e EMAIL_USERNAME="$EMAIL_USERNAME"  -e EMAIL_PASSWORD="$EMAIL_PASSWORD"  -e SENDER_NAME="$SENDER_NAME"   -e SENDER_EMAIL="$SENDER_EMAIL"  -e BREVO_API_KEY="$BREVO_API_KEY" -p 80:8080 stykle/brevo:latest
